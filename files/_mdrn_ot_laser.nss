...
void main()
{
    ...
    if (GetBaseItemType(oGun)==8 || GetBaseItemType(oGun)==7)
    {
         ...
         object oNewGun = CopyItemAndModify(oGun, ITEM_APPR_TYPE_WEAPON_MODEL, ITEM_APPR_WEAPON_MODEL_TOP, 7, TRUE);
         AssignCommand(oPC, ClearAllActions());
         AssignCommand(oPC, ActionEquipItem(oNewGun, INVENTORY_SLOT_RIGHTHAND));
         DestroyObject(oGun, 1.0);
         AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAttackBonus(2), oNewGun);

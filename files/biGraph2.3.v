...
match m with
|nil => True
|e::m' => ~(In (src gra e) vl) /\ ~(In (dst gra e) vl) /\ is_matching vl m'
end.
(*v1是m中所有点的集合*)
...
Definition E_eqb (e1 e2: E) : bool := 
  if Nat.eqb (src gra e1) (src gra e2) then
    ...
    if Nat.eqb (src gra e1) (dst gra e2) then
      if Nat.eqb (dst gra e1) (src gra e2) then
      true
...
Fixpoint exclude_edge (e1: list E) (p: list E) : list E :=
match p with
| nil => e1
| e::_ as p' => if (Inb_edge e e1) then exclude_edge e1 p'
    else e::exclude_edge e1 p'
end.

Definition xor_edge (e1 p: list E) : list E :=
  exclude_edge e1 p ++ exclude_edge p e1.

Lemma match_xor_aug_match:
  forall (e p: list E) (v: V) (v1 v2: list V), aug_path p-> v1 = epath_to_vpath gra M -> is_matching v1 M  -> e = xor_edge M p -> v2 = epath_to_vpath gra e -> is_matching v2 e.
Proof.
  intros.
  unfold aug_path in H.
  destruct p.
  unfold xor_edge in H2.
  unfold exclude_edge in H2.
  rewrite H3.
  rewrite H0 in H1.
  Admitted.

Theorem greatermatch_to_aug: 
forall (M': list E) (v1 v2: list V) , v1 = epath_to_vpath gra M -> v2 = epath_to_vpath gra M' -> length M' > length M -> is_matching v2 M' -> is_matching v1 M -> exists p: list E, aug_path p.
Proof.
  intros.

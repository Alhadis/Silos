...
(******** ANSWER: 
   The reflexivity tactic simplifies the 0 + n clause, by expanding the def
   of plus. It can't do this with n + 0, because it can't automatically
   apply commutivity.
****************)
...
(* Destruct performs case analysis by breaking down the proof
   into the possible top level structure of the input value.
   Induction proves that a property holds true for all
   values if it can be shows that it is true for one value
   implies it is true for all. *)
...
Theorem unary_2n_is_double : forall n : nat, 
  unary_to_bin (n + n) =
...
Theorem bd_inc_eq_bn : forall b : bin_nat,
  bin_inc
  (match b with
  | BO => BO
  | BD b' => BD (BD b')
...
Proof.
  ...
  simpl. rewrite <- IHb'.
    ...
    rewrite -> unary_2n_is_double. reflexivity.
    rewrite -> plus_comm. reflexivity.
  simpl. rewrite <- IHb'.
    replace (bin_to_unary b' + 0) with (bin_to_unary b').
    rewrite -> unary_2n_is_double. rewrite -> bd_inc_eq_bn. reflexivity.
    rewrite -> plus_comm. reflexivity.
Qed.
...
(**
  ...
  Fixpoint add_then_sub (n: nat) : nat :=
  match n with
  | O => 0
  | S n' => S (match evenb n' with
           | true => add_then_sub (n + 1)
           | false =>  add_then_sub (n - 2)
           end)
  end.
*)
(** [] *)

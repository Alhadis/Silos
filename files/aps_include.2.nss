...
int SQLFetch()
{
    //debugVarObject("SQLFetch()", OBJECT_SELF);
    string sRow;
...
string SQLGetData(int iCol)
{
    //debugVarObject("SQLGetData()", OBJECT_SELF);
    int iPos;
    string sResultSet = GetLocalString(GetModule(), "NWNX_ODBC_CurrentRow");
    //debugVarString("sResultSet", sResultSet);
    ...
    //debugVarString("sColValue", sColValue);
    return sColValue;
}

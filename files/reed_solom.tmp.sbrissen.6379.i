# 1 "../../lib/reed_solomon/reed_solomon.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../lib/reed_solomon/reed_solomon.c"
# 41 "../../lib/reed_solomon/reed_solomon.c"
...
# 42 "../../lib/reed_solomon/reed_solomon.c" 2
...
# 43 "../../lib/reed_solomon/reed_solomon.c" 2
...
# 44 "../../lib/reed_solomon/reed_solomon.c" 2
...
# 45 "../../lib/reed_solomon/reed_solomon.c" 2
# 1 "../../include/linux/rslib.h" 1
# 41 "../../include/linux/rslib.h"
struct rs_control {
 int mm;
 int nn;
 uint16_t *alpha_to;
...
# 100 "../../include/linux/rslib.h"
static inline __attribute__((always_inline)) int rs_modnn(struct rs_control *rs, int x)
{
 while (x >= rs->nn) {
  x -= rs->nn;
  x = (x >> rs->mm) + (x & rs->nn);
...
# 46 "../../lib/reed_solomon/reed_solomon.c" 2
...
# 47 "../../lib/reed_solomon/reed_solomon.c" 2



static struct list_head rslist = { &(rslist), &(rslist) };

static struct mutex rslistlock = { .count = { (1) } , .wait_lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } } , .wait_list = { &(rslistlock.wait_list), &(rslistlock.wait_list) } };
# 66 "../../lib/reed_solomon/reed_solomon.c"
                                  ...
                                  int fcr, int prim, int nroots)
{
 ...
 rs = kmalloc(sizeof (struct rs_control), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (rs == ((void *)0))
  return ((void *)0);
 ...
 rs->alpha_to = kmalloc(sizeof(uint16_t) * (rs->nn + 1), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (rs->alpha_to == ((void *)0))
  goto errrs;

 rs->index_of = kmalloc(sizeof(uint16_t) * (rs->nn + 1), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (rs->index_of == ((void *)0))
  goto erralp;

 rs->genpoly = kmalloc(sizeof(uint16_t) * (rs->nroots + 1), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
...
# 199 "../../lib/reed_solomon/reed_solomon.c"
                                           ...
                                           int prim, int nroots)
{
 ...
 for (tmp = (&rslist)->next; prefetch(tmp->next), tmp != (&rslist); tmp = tmp->next) {
  rs = ({ const typeof( ((struct rs_control *)0)->list ) *__mptr = (tmp); (struct rs_control *)( (char *)__mptr - __builtin_offsetof(struct rs_control,list) );});
  if (symsize != rs->mm)
   continue;
  if (gfpoly != rs->gfpoly)
   continue;
...
# 260 "../../lib/reed_solomon/reed_solomon.c"
struct rs_control *init_rs(int symsize, int gfpoly, int fcr, int prim,
                           int nroots)
{
 return init_rs_internal(symsize, gfpoly, ((void *)0), fcr, prim, nroots);
}
# 279 "../../lib/reed_solomon/reed_solomon.c"
struct rs_control *init_rs_non_canonical(int symsize, int (*gffunc)(int),
                                         int fcr, int prim, int nroots)
{
 return init_rs_internal(symsize, 0, gffunc, fcr, prim, nroots);
}
# 298 "../../lib/reed_solomon/reed_solomon.c"
int encode_rs8(struct rs_control *rs, uint8_t *data, int len, uint16_t *par,
        uint16_t invmsk)
{
# 1 "../../lib/reed_solomon/encode_rs.c" 1
# 20 "../../lib/reed_solomon/encode_rs.c"
{
 int i, j, pad;
 int nn = rs->nn;
...
# 302 "../../lib/reed_solomon/reed_solomon.c" 2
}
extern typeof(encode_rs8) encode_rs8; static const char __kstrtab_encode_rs8[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "encode_rs8"; static const struct kernel_symbol __ksymtab_encode_rs8 __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&encode_rs8, __kstrtab_encode_rs8 };
# 324 "../../lib/reed_solomon/reed_solomon.c"
int decode_rs8(struct rs_control *rs, uint8_t *data, uint16_t *par, int len,
        uint16_t *s, int no_eras, int *eras_pos, uint16_t invmsk,
        uint16_t *corr)
{
# 1 "../../lib/reed_solomon/decode_rs.c" 1
# 19 "../../lib/reed_solomon/decode_rs.c"
{
 ...
 pad = nn - nroots - len;
 do { if (__builtin_expect(!!(pad < 0 || pad >= nn), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 decode:
 ({ void *__p = (&lambda[1]); size_t __n = nroots * sizeof(lambda[0]); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 lambda[0] = 1;
 ...
 for (j = count - 1; j >= 0; j--) {
  ...
  for (i = ({ typeof(deg_lambda) _min1 = (deg_lambda); typeof(nroots - 1) _min2 = (nroots - 1); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; }) & ~1; i >= 0; i -= 2) {
   if (lambda[i + 1] != nn) {
...
# 329 "../../lib/reed_solomon/reed_solomon.c" 2
}
extern typeof(decode_rs8) decode_rs8; static const char __kstrtab_decode_rs8[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "decode_rs8"; static const struct kernel_symbol __ksymtab_decode_rs8 __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&decode_rs8, __kstrtab_decode_rs8 };
# 377 "../../lib/reed_solomon/reed_solomon.c"
extern typeof(init_rs) init_rs; static const char __kstrtab_init_rs[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "init_rs"; static const struct kernel_symbol __ksymtab_init_rs __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&init_rs, __kstrtab_init_rs };
extern typeof(init_rs_non_canonical) init_rs_non_canonical; static const char __kstrtab_init_rs_non_canonical[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "init_rs_non_canonical"; static const struct kernel_symbol __ksymtab_init_rs_non_canonical __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&init_rs_non_canonical, __kstrtab_init_rs_non_canonical };
extern typeof(free_rs) free_rs; static const char __kstrtab_free_rs[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "free_rs"; static const struct kernel_symbol __ksymtab_free_rs __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&free_rs, __kstrtab_free_rs };

...
; Fread handle,dest,len  (ret: D0.l Tatsächliche Länge)
Fread	MACRO
	pea	\2
	move.l	\3,-(sp)
	move.w	\1,-(sp)
...
; Fwrite handle,source,len  (ret: d0.l Tatsächliche Anzahl geschriebener Bytes)
Fwrite	MACRO
	pea	\2
	move.l	\3,-(sp)
	move.w	\1,-(sp)
...
; _LOAD fname^,dest,len  (ret: d0.l Tatsächliche Länge, neg.l:Fehler)
_LOAD	MACRO
...
;    a0.l  Fname^
;    a1.l  Dest
; <- d0.l  Tatsächliche Länge oder Fehler (negativ LONG!)
load:	move.l	d0,d6
	Fopen	(a0)
	move.w	d0,d7
	bmi.s	.e

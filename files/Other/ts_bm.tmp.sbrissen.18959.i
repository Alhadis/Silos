...
static unsigned int bm_find(struct ts_config *conf, struct ts_state *state)
{
 ...
 const u8 icase = conf->flags & 2;

 for (;;) {
  text_len = conf->get_next_block(consumed, &text, conf, state);

  if (__builtin_expect(!!(text_len == 0), 0))
   break;
  ...
  while (shift < text_len) {
   ...
   for (i = 0; i < bm->patlen; i++)
    if ((icase ? __toupper(text[shift-i])
        : text[shift-i])
...
next: bs = bm->bad_shift[text[shift-i]];
   ...
   shift = ({ int __max1 = (shift-i+bs); int __max2 = (shift+bm->good_shift[i]); __max1 > __max2 ? __max1: __max2; });
  }
  consumed += text_len;
 }
...
static void compute_prefix_tbl(struct ts_bm *bm, int flags)
{
 ...
 for (i = 0; i < bm->patlen - 1; i++) {
  ...
  if (flags & 2)
   bm->bad_shift[__tolower(bm->pattern[i])]
       = bm->patlen - 1 - i;
 }
     ...
     gfp_t gfp_mask, int flags)
{
 ...
 if (flags & 2)
  for (i = 0; i < len; i++)
   bm->pattern[i] = __toupper(((u8 *)pattern)[i]);
 else
...
static struct ts_ops bm_ops = {
 ...
 .list = { &(bm_ops.list), &(bm_ops.list) }
};

static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) init_bm(void)
{
 return textsearch_register(&bm_ops);
}

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) exit_bm(void)
{
...
static initcall_t __initcall_init_bm6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = init_bm;;
static exitcall_t __exitcall_exit_bm __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = exit_bm;;

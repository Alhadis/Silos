...
# 28 "../sysdeps/generic/inttypes.h" 2
...
# 32 "dl-profile.c" 2
# 1 "../include/sys/gmon.h" 1

# 1 "../gmon/sys/gmon.h" 1
# 37 "../gmon/sys/gmon.h"
# 1 "../include/sys/types.h" 1
# 38 "../gmon/sys/gmon.h" 2
# 46 "../gmon/sys/gmon.h"
struct __bb
{
  long zero_word;
...
extern void __monstartup (unsigned long __lowpc, unsigned long __highpc) __attribute__ ((__nothrow__ ));
extern void monstartup (unsigned long __lowpc, unsigned long __highpc) __attribute__ ((__nothrow__ ));


extern void _mcleanup (void) __attribute__ ((__nothrow__ ));


# 3 "../include/sys/gmon.h" 2
...
extern struct gmonparam _gmonparam __attribute__ ((visibility ("hidden")));
# 33 "dl-profile.c" 2
# 1 "../include/sys/gmon_out.h" 1
# 1 "../gmon/sys/gmon_out.h" 1
# 38 "../gmon/sys/gmon_out.h"
...
_dl_start_profile (void)
{
  ...
  struct stat64 st;
  const Elf64_Phdr *ph;
  Elf64_Addr mapstart = ~((Elf64_Addr) 0);
  Elf64_Addr mapend = 0;
  char *hist, *cp;
  size_t idx;
  size_t tossize;
...
# 199 "dl-profile.c"
  ...
  const char *errstr = 
# 204 "dl-profile.c" 3 4
                      ((void *)0)
# 204 "dl-profile.c"
                          ;


  for (ph = _rtld_local._dl_profile_map->l_phdr;
       ph < &_rtld_local._dl_profile_map->l_phdr[_rtld_local._dl_profile_map->l_phnum]; ++ph)
    if (ph->p_type == 1 && (ph->p_flags & (1 << 0)))
      {
 Elf64_Addr start = (ph->p_vaddr & ~(_rtld_local_ro._dl_pagesize - 1));
 Elf64_Addr end = ((ph->p_vaddr + ph->p_memsz + _rtld_local_ro._dl_pagesize - 1)
 ...
 if (end > mapend)
  ...
  running = 0;
  lowpc = (((mapstart + _rtld_local._dl_profile_map->l_addr)/(2 * sizeof (unsigned short)))*(2 * sizeof (unsigned short)))
                                           ;
  highpc = ((((mapend + _rtld_local._dl_profile_map->l_addr)+(2 * sizeof (unsigned short))-1)/(2 * sizeof (unsigned short)))*(2 * sizeof (unsigned short)))
                                          ;
  textsize = highpc - lowpc;
  kcountsize = textsize / 2;
  if ((2 & (2 - 1)) == 0)
    {
      ...
      ((void) sizeof ((2 == 2) ? 1 : 0), __extension__ ({ if (2 == 2) ; else __assert_fail ("HASHFRACTION == 2", "dl-profile.c", 239, __extension__ __PRETTY_FUNCTION__); }));
 ...
 log_hashfraction = __ffs (2 * sizeof (*froms)) - 1;
    }
  else
    log_hashfraction = -1;
  tossize = textsize / 2;
  fromlimit = textsize * 3 / 100;
  if (fromlimit < 50)
    fromlimit = 50;
  if (fromlimit > (1 << 20))
    fromlimit = (1 << 20);
...
# 273 "dl-profile.c"
  ...
  memcpy (&gmon_hdr.cookie[0], "gmon", sizeof (gmon_hdr.cookie));
  gmon_hdr.version = 0x1ffff;
...
# 302 "dl-profile.c"
  ...
  hist_hdr.hist_size = kcountsize / sizeof (unsigned short);
  ...
  filename = (char *) __builtin_alloca (strlen (_rtld_local_ro._dl_profile_output) + 1 + strlen (_rtld_local_ro._dl_profile) + sizeof ".profile")
                                                         ;
  cp = __builtin_stpcpy (filename, _rtld_local_ro._dl_profile_output);
  *cp++ = '/';
  __builtin_stpcpy (__builtin_stpcpy (cp, _rtld_local_ro._dl_profile), ".profile");

  fd = __open64_nocancel (filename, 02|0100|0400000, (0400|0200|(0400 >> 3)|(0200 >> 3)|((0400 >> 3) >> 3)|((0200 >> 3) >> 3)));
  if (fd == -1)
    {
      char buf[400];
      int errnum;
    ...
    print_error:
      ...
      if (fd != -1)
 __close_nocancel (fd);
      _dl_dprintf (2, errstr, filename, __strerror_r (errnum, buf, sizeof buf))
                                          ;
      return;
    }

  if (__fxstat64 (1, fd, &st) < 0 || !((((st.st_mode)) & 0170000) == (0100000)))
    {
  ...
  if (st.st_size == 0)
    {
      ...
      char buf[_rtld_local_ro._dl_pagesize];

      memset (buf, '\0', _rtld_local_ro._dl_pagesize);

      if (__lseek (fd, expected_size & ~(_rtld_local_ro._dl_pagesize - 1), 0) == -1)
 {
 cannot_create:
   ...
   goto print_error;
      ...
      if ((__extension__ ({ long int __result; do __result = (long int) (__write_nocancel (fd, buf, (expected_size & (_rtld_local_ro._dl_pagesize - 1)))); while (__result == -1L && rtld_errno == 
# 368 "dl-profile.c" 3 4
         4
# 368 "dl-profile.c"
         ); __result; }))
 ...
 goto cannot_create;
    ...
    wrong_format:
      ...
      if (addr != 
# 378 "dl-profile.c" 3 4
                 ((void *)0)
# 378 "dl-profile.c"
                     )
 __munmap ((void *) addr, expected_size);

      _dl_dprintf (2, "%s: file is no correct profile data file for `%s'\n", filename, _rtld_local_ro._dl_profile)
                              ;
      return;
    }

  addr = (struct gmon_hdr *) __mmap (
# 386 "dl-profile.c" 3 4
                                    ((void *)0)
# 386 "dl-profile.c"
                                        , expected_size, 0x1|0x2,
         0x01|0, fd, 0);
  if (addr == (struct gmon_hdr *) ((void *) -1))
    {
 ...
 goto wrong_format;
  ...
  if (tos == 
# 430 "dl-profile.c" 3 4
            ((void *)0)
# 430 "dl-profile.c"
                )
    {
      __munmap ((void *) addr, expected_size);
      do { _dl_dprintf (2, "Out of memory while initializing profiler\n"); _exit (127); } while (1);
  ...
  fromidx = 0;
# 447 "dl-profile.c"
  for (idx = narcs = (((*narcsp)<(fromlimit))?(*narcsp):(fromlimit)); idx > 0; )
    {
      size_t to_index;
      size_t newfromidx;
      --idx;
      to_index = (data[idx].self_pc / (2 * sizeof (*tos)));
      newfromidx = fromidx++;
  ...
  if (kcountsize < highpc - lowpc)
    {
      ...
      if (quot >= 0x10000L)
 s_scale = 1;
      else if (quot >= 0x10000L / 256)
 s_scale = 0x10000L / quot;
      else if (range > 
# 472 "dl-profile.c" 3 4
                      (0x7fffffffffffffffL * 2UL + 1UL) 
# 472 "dl-profile.c"
                                / 256)
 s_scale = (0x10000L * 256) / (range / (kcountsize / 256));
      else
 s_scale = (0x10000L * 256) / ((range * 256) / kcountsize);
...
void
_dl_mcount (Elf64_Addr frompc, Elf64_Addr selfpc)
{
 ...
 check_new_or_add:
   ...
   while (narcs != *narcsp && narcs < fromlimit)
     ...
     / (2 * sizeof (*tos)));
       newfromidx = ({ __typeof (*&fromidx) result; if (sizeof (*&fromidx) == 1) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddb %b0, %1" : "=q" (result), "=m" (*&fromidx) : "0" (1), "m" (*&fromidx), "i" (
# 552 "dl-profile.c" 3 4
                   __builtin_offsetof (
# 552 "dl-profile.c"
                   tcbhead_t
# 552 "dl-profile.c" 3 4
...
# 552 "dl-profile.c"
                   )); else if (sizeof (*&fromidx) == 2) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddw %w0, %1" : "=r" (result), "=m" (*&fromidx) : "0" (1), "m" (*&fromidx), "i" (
# 552 "dl-profile.c" 3 4
                   __builtin_offsetof (
# 552 "dl-profile.c"
                   tcbhead_t
...
# 552 "dl-profile.c"
                   )); else if (sizeof (*&fromidx) == 4) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddl %0, %1" : "=r" (result), "=m" (*&fromidx) : "0" (1), "m" (*&fromidx), "i" (
# 552 "dl-profile.c" 3 4
                   __builtin_offsetof (
# 552 "dl-profile.c"
                   tcbhead_t
...
# 552 "dl-profile.c"
                   )); else __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddq %q0, %1" : "=r" (result), "=m" (*&fromidx) : "0" ((atomic64_t) ((__typeof__ (*(0 ? (__typeof__ (0 ? (__typeof__ ((__typeof__ (1)) 0) *) 0 : (void *) ((__builtin_classify_type ((__typeof__ (1)) 0) == 5)))) 0 : (__typeof__ (0 ? (intptr_t *) 0 : (void *) (!((__builtin_classify_type ((__typeof__ (1)) 0) == 5))))) 0))) (1))), "m" (*&fromidx), "i" (
# 552 "dl-profile.c" 3 4
                   __builtin_offsetof (
# 552 "dl-profile.c"
                   tcbhead_t
...
# 552 "dl-profile.c"
       ...
       froms[newfromidx].link = tos[to_index];
       tos[to_index] = newfromidx;
       do { if (sizeof (*&narcs) == 1) __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incb %b0" : "=m" (*&narcs) : "m" (*&narcs), "i" (
# 556 "dl-profile.c" 3 4
      __builtin_offsetof (
# 556 "dl-profile.c"
      tcbhead_t
# 556 "dl-profile.c" 3 4
...
# 556 "dl-profile.c"
      )); else if (sizeof (*&narcs) == 2) __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incw %w0" : "=m" (*&narcs) : "m" (*&narcs), "i" (
# 556 "dl-profile.c" 3 4
      __builtin_offsetof (
# 556 "dl-profile.c"
      tcbhead_t
...
# 556 "dl-profile.c"
      )); else if (sizeof (*&narcs) == 4) __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incl %0" : "=m" (*&narcs) : "m" (*&narcs), "i" (
# 556 "dl-profile.c" 3 4
      __builtin_offsetof (
# 556 "dl-profile.c"
      tcbhead_t
...
# 556 "dl-profile.c"
      )); else __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incq %q0" : "=m" (*&narcs) : "m" (*&narcs), "i" (
# 556 "dl-profile.c" 3 4
      __builtin_offsetof (
# 556 "dl-profile.c"
      tcbhead_t
...
# 556 "dl-profile.c"
   ...
   if (*topcindex == 0)
     {
       uint_fast32_t newarc = ({ __typeof (*narcsp) result; if (sizeof (*narcsp) == 1) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddb %b0, %1" : "=q" (result), "=m" (*narcsp) : "0" (1), "m" (*narcsp), "i" (
# 562 "dl-profile.c" 3 4
                             __builtin_offsetof (
# 562 "dl-profile.c"
                             tcbhead_t
# 562 "dl-profile.c" 3 4
...
# 562 "dl-profile.c"
                             )); else if (sizeof (*narcsp) == 2) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddw %w0, %1" : "=r" (result), "=m" (*narcsp) : "0" (1), "m" (*narcsp), "i" (
# 562 "dl-profile.c" 3 4
                             __builtin_offsetof (
# 562 "dl-profile.c"
                             tcbhead_t
...
# 562 "dl-profile.c"
                             )); else if (sizeof (*narcsp) == 4) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddl %0, %1" : "=r" (result), "=m" (*narcsp) : "0" (1), "m" (*narcsp), "i" (
# 562 "dl-profile.c" 3 4
                             __builtin_offsetof (
# 562 "dl-profile.c"
                             tcbhead_t
...
# 562 "dl-profile.c"
                             )); else __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddq %q0, %1" : "=r" (result), "=m" (*narcsp) : "0" ((atomic64_t) ((__typeof__ (*(0 ? (__typeof__ (0 ? (__typeof__ ((__typeof__ (1)) 0) *) 0 : (void *) ((__builtin_classify_type ((__typeof__ (1)) 0) == 5)))) 0 : (__typeof__ (0 ? (intptr_t *) 0 : (void *) (!((__builtin_classify_type ((__typeof__ (1)) 0) == 5))))) 0))) (1))), "m" (*narcsp), "i" (
# 562 "dl-profile.c" 3 4
                             __builtin_offsetof (
# 562 "dl-profile.c"
                             tcbhead_t
...
# 562 "dl-profile.c"
  ...
  goto done;
       ...
       *topcindex = ({ __typeof (*&fromidx) result; if (sizeof (*&fromidx) == 1) __asm __volatile ("cmpl $0, %%fs:%P4\n\tje 0f\n\tlock\n0:\t" "xaddb %b0, %1" : "=q" (result), "=m" (*&fromidx) : "0" (1), "m" (*&fromidx), "i" (
# 569 "dl-profile.c" 3 4
                   __builtin_offsetof (
# 569 "dl-profile.c"
                   tcbhead_t
# 569 "dl-profile.c" 3 4
...
# 577 "dl-profile.c"
 ...
 break;
  ...
  do { if (sizeof (*&fromp->here->count) == 1) __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incb %b0" : "=m" (*&fromp->here->count) : "m" (*&fromp->here->count), "i" (
# 590 "dl-profile.c" 3 4
 __builtin_offsetof (
# 590 "dl-profile.c"
 tcbhead_t
# 590 "dl-profile.c" 3 4
...
# 590 "dl-profile.c"
 )); else if (sizeof (*&fromp->here->count) == 2) __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incw %w0" : "=m" (*&fromp->here->count) : "m" (*&fromp->here->count), "i" (
# 590 "dl-profile.c" 3 4
 __builtin_offsetof (
# 590 "dl-profile.c"
 tcbhead_t
...
# 590 "dl-profile.c"
 )); else if (sizeof (*&fromp->here->count) == 4) __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incl %0" : "=m" (*&fromp->here->count) : "m" (*&fromp->here->count), "i" (
# 590 "dl-profile.c" 3 4
 __builtin_offsetof (
# 590 "dl-profile.c"
 tcbhead_t
...
# 590 "dl-profile.c"
 )); else __asm __volatile ("cmpl $0, %%fs:%P2\n\tje 0f\n\tlock\n0:\t" "incq %q0" : "=m" (*&fromp->here->count) : "m" (*&fromp->here->count), "i" (
# 590 "dl-profile.c" 3 4
 __builtin_offsetof (
# 590 "dl-profile.c"
 tcbhead_t
...
extern __typeof (_dl_mcount) __EI__dl_mcount __asm__("" "_dl_mcount"); extern __typeof (_dl_mcount) __EI__dl_mcount __attribute__((alias ("" "__GI__dl_mcount")));

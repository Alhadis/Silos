...
begin
  ...
  else if (falling_edge) bit_count <= bit_count + 1'b1;
...
begin
  if (reset || rising_edge || falling_edge) watchdog_timer_count <= 0;
    ...
    watchdog_timer_count <= watchdog_timer_count + 1'b1;
end
assign watchdog_timer_done = (watchdog_timer_count==WATCHDOG_TIMER_VALUE_PP-1'b1);
 
// This is the debounce timer counter
always @(posedge clk)
begin
  if (reset || falling_edge || rising_edge) debounce_timer_count <= 1'b0;
//  else if (~debounce_timer_done)
  else debounce_timer_count <= debounce_timer_count + 1'b1;
end

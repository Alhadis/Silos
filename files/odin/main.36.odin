...
read_instruction :: proc(instr: string) -> (string, int, int) {
    instr := instr;
    dir := instr[0];
    instr = instr[1:];

    dist := 0;
    for instr != "" && instr[0] >= '0' && instr[0] <= '9' {
        dist *= 10;
        dist += int(instr[0]) - '0';
        instr = instr[1:];
    }

    if instr != "" && instr[0] == ',' do instr = instr[1:];
...
part2_get_intersections :: proc(a, b: string) -> []int {
    a := a;
    b := b;
    intersections: [dynamic]int;
    ...
    a_lines: [dynamic]Stored_Point;
    defer delete(a_lines);

    a_pos: [2]int;
    a_steps := 0;
    for a != "" {
        x, y: int;
        a, x, y = read_instruction(a);
        old_pos := a_pos;
        a_pos.x += x;
        a_pos.y += y;
        ...
        for old_pos != a_pos {
            ...
            append(&a_lines, Stored_Point{old_pos, a_steps});
        }
    }
    ...
    for b != "" {
        ...
        for old_pos != b_pos {
            ...
            for p in a_lines {
                if old_pos == p.pos do append(&intersections, p.steps+b_steps);
            }
        }
...
part1_get_intersections :: proc(a, b: string) -> [][2]int {
    a := a;
    b := b;
    intersections: [dynamic][2]int;

    a_lines: [dynamic][2]int;
    defer delete(a_lines);
    ...
    for b != "" {
        ...
        for old_pos != b_pos {
            ...
            for p in a_lines {
                if old_pos == p do append(&intersections, p);
            }
        }
...
part1 :: proc() {
    intersections := part1_get_intersections(test_wire_1, test_wire_2);
    shortest := 99999;
    ...
    for i in intersections {
        ...
        if dist < shortest do shortest = dist;
    }
...
main :: proc() {
    intersections := part2_get_intersections(wire_1, wire_2);
    most_efficient := 99999;

    for i in intersections {
        if i < most_efficient do most_efficient = i;
    }

    fmt.printf("most efficient intersection: %v\n", most_efficient);
}

test_wire_1 := "R75,D30,R83,U83,L12,D49,R71,U7,L72";
test_wire_2 := "U62,R66,U55,R34,D71,R55,D58,R83";

test_wire_2_1 := "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
test_wire_2_2 := "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";

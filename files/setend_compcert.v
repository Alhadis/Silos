...
Definition E := 47.
Definition proc := 48.
Definition StatusRegister_to_uint32 := 49.
Definition set_bit := 50.
Definition SETEND := 51.
...
Definition T1 := Tfunction T[`*` uint32; uint32; uint8] void.
Definition T2 := Tfunction T[`*` typ_SLv6_StatusRegister] uint32.
Definition T3 := `*` typ_SLv6_Processor.
Definition T4 := typ_SLv6_Processor.
...
Definition fun_internal_SETEND :=
  {| fn_return := void;
     fn_params := [
...
E -: int8];
     fn_vars := [];
     fn_body :=
(call (\set_bit`:T1) E[(call (\StatusRegister_to_uint32`:T2) E[&((`*(\proc`:T3)`:T4)|cpsr`:T5)`:T5] T6); #9`:T7; \E`:T8] T9) |}.

Definition fun_SETEND :=
  (SETEND, Internal fun_internal_SETEND).

Definition functions : list (prod ident fundef) := [fun_SETEND].

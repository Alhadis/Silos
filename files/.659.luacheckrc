std = "lua51"
max_line_length = false
exclude_files = {
    "libs/",
    ".luacheckrc"
}

ignore = {
    "211", -- Unused local variable
    "212", -- Unused argument
    "213", -- Unused loop variable
    "311", -- Value assigned to a local variable is unused
    "432", -- Shadowing an upvalue argument
    "542", -- empty if branch
}

globals = {
    "AcceptQuest",
    "AcceptResurrect",
    "BINDING_HEADER_IDTBAR1",
    "BINDING_HEADER_IDTBAR2",
    "BINDING_HEADER_IDTBAR3",
    "BINDING_HEADER_IDTBAR4",
    "BINDING_HEADER_IDTBAR5",
    "BINDING_HEADER_IDTBAR5",
    "BINDING_HEADER_IDTPETBAR",
    "BuffFrame",
    "C_DeathInfo",
    "CanMerchantRepair",
    "CastingBarFrame",
    "ChatFrame_AddMessageEventFilter",
    "ClearCursor",
    "CompleteQuest",
    "ConfirmSummon",
    "DeleteCursorItem",
    "DevTools_Dump",
    "Dismount",
    "ERR_ATTACK_MOUNTED",
    "ERR_MOUNT_ALREADYMOUNTED",
    "ERR_NOT_WHILE_MOUNTED",
    "ERR_TAXIPLAYERALREADYMOUNTED",
    "GameTimeFrame",
    "GetCoinTextureString",
    "GetContainerItemLink",
    "GetContainerNumSlots",
    "GetCorpseRecoveryDelay",
    "GetMinimapShape",
    "GetMoney",
    "GetNumQuestChoices",
    "GetNumQuestLogEntries",
    "GetQuestIndexForWatch",
    "GetQuestLogSelection",
    "GetQuestLogTitle",
    "GetRepairAllCost",
    "GetRuneCooldown",
    "GetSendMailItem",
    "GetXPExhaustion",
    "GossipTitleButton1",
    "GossipTitleButton2",
    "IsMounted",
    "IsQuestCompletable",
    "MainMenuBar",
    "MiniMapWorldMapButton",
    "MinimapBorder",
    "MinimapBorderTop",
    "MinimapCluster",
    "MinimapNorthTag",
    "MinimapToggleButton",
    "MinimapZoneText",
    "MinimapZoneTextButton",
    "MinimapZoomIn",
    "MinimapZoomOut",
    "MultiBarLeft",
    "MultiBarRight",
    "PetActionBarFrame",
    "PetHasActionBar",
    "PickupContainerItem",
    "QuestFrameCompleteQuestButton",
    "RegisterStateDriver",
    "ReloadUI",
    "RepairAllItems",
    "RepopMe",
    "SLASH_IDTWEAKS_RELOAD1",
    "SLASH_IDTWEAKS_RELOAD1",
    "SLASH_IDTWEAKS_RELOAD2",
    "SLASH_IDTWEAKS_RELOAD2",
    "SPELL_FAILED_NOT_MOUNTED",
    "SelectQuestLogEntry",
    "SendMail",
    "SendMailNameEditBox",
    "SendMailScrollChildFrame",
    "SendMailSubjectEditBox",
    "ShowQuestComplete",
    "SlashCmdList",
    "SlidingActionBarTexture0",
    "SlidingActionBarTexture1",
    "StaticPop1Button1",
    "StaticPopup1",
    "StaticPopup1Button1",
    "TimeManagerClockButton",
    "UIErrorsFrame",
    "UnitAffectingCombat",
    "UnitClass",
    "UnitXP",
    "UnitXP",
    "UnitXPMax",
    "_",
    "_G",
    "idTweaks",
    "idTweaksActionBar1",
    "idTweaksActionBar1Button1",
    "idTweaksDB",
    "idTweaksExperienceBar",
    "itemName",
    "nothing",
}

read_globals = {
    "bit", "ceil", "floor", "mod", "max", "table", "tinsert", "tContains",
    "wipe", "copy", "string", "tostringall", "strtrim", "strmatch", "strjoin",
    "strsplit", "strlower", "time",

    -- our own globals

    -- misc custom, third party libraries
    "LibStub",
    -- "tekDebug",
    -- "AceGUIWidgetLSMlists",
    -- "ChatThrottleLib",
    -- "GetAuctionBuyout",
    -- "TomTom",

    -- API functions
    "C_ChatInfo",
    "C_Map",
    "C_Timer",
    "C_VignetteInfo",
    "Debug",
    "Enum",
    "BankButtonIDToInvSlotID",
    "ContainerIDToInventoryID",
    "ReagentBankButtonIDToInvSlotID",
    "Ambiguate",
    "CheckInteractDistance",
    "CreateMacro",
    "CursorHasItem",
    "DeleteCursorItem",
    "FlashClientIcon",
    "GetAddOnMetadata",
    "GetAchievementCriteriaInfo",
    "GetAchievementCriteriaInfoByID",
    "GetAchievementInfo",
    "GetAchievementNumCriteria",
    "GetAtlasInfo",
    "GetAuctionItemSubClasses",
    "GetBuildInfo",
    "GetBackpackAutosortDisabled",
    "GetBagSlotFlag",
    "GetBankAutosortDisabled",
    "GetBankBagSlotFlag",
    "GetContainerNumFreeSlots",
    "GetContainerNumSlots",
    "GetContainerItemID",
    "GetContainerItemInfo",
    "GetContainerItemLink",
    "GetCurrentGuildBankTab",
    "GetCurrentMapAreaID",
    "GetCursorInfo",
    "GetGuildBankItemInfo",
    "GetGuildBankItemLink",
    "GetGuildBankTabInfo",
    "GetGuildBankNumSlots",
    "GetInventoryItemLink",
    "GetItemClassInfo",
    "GetItemFamily",
    "GetItemInfo",
    "GetItemInfoInstant",
    "GetItemQualityColor",
    "GetMacroIndexByName",
    "GetNumMacros",
    "GetRaidTargetIndex",
    "GetTime",
    "InCombatLockdown",
    "IsAddOnLoaded",
    "IsAltKeyDown",
    "IsControlKeyDown",
    "IsInGroup",
    "IsInGuild",
    "IsInInstance",
    "IsInRaid",
    "IsModifierKeyDown",
    "IsQuestFlaggedCompleted",
    "IsReagentBankUnlocked",
    "IsShiftKeyDown",
    "LoadAddOn",
    "PlaySoundFile",
    "PickupContainerItem",
    "PickupGuildBankItem",
    "QueryGuildBankTab",
    "SetMapByID",
    "SetMapToCurrentZone",
    "SetRaidTarget",
    "SetRaidTargetIconTexture",
    "SplitContainerItem",
    "SplitGuildBankItem",
    "UnitClassification",
    "UnitCreatureType",
    "UnitExists",
    "UnitFactionGroup",
    "UnitGUID",
    "UnitIsAFK",
    "UnitIsDead",
    "UnitIsVisible",
    "UnitLevel",
    "UnitName",
    "UnitOnTaxi",
    "UnitPlayerControlled",
    "UseContainerItem",

    -- FrameXML frames
    "BankFrame",
    "MerchantFrame",
    "GameTooltip",
    "UIParent",
    "WorldFrame",
    "Minimap",
    "DEFAULT_CHAT_FRAME",
    "GameFontHighlightSmall",

    -- FrameXML API
    "CreateFrame",
    "CreateFramePool",
    "InterfaceOptionsFrame_OpenToCategory",
    "CloseDropDownMenus",
    "ToggleDropDownMenu",
    "UIDropDownMenu_AddButton",
    "UISpecialFrames",
    "ScrollingEdit_OnCursorChanged",
    "ScrollingEdit_OnUpdate",
    "GameTooltip_SetTitle",

    -- FrameXML Constants
    "ACTION_PARTY_KILL",
    "ADD",
    "BACKPACK_CONTAINER",
    "BACKPACK_TOOLTIP",
    "BAG_CLEANUP_BAGS",
    "BAG_FILTER_ICONS",
    "BAGSLOT",
    "BANK",
    "BANK_BAG_PURCHASE",
    "BANK_CONTAINER",
    "CLOSE",
    "COMPLETE",
    "CONFIRM_BUY_BANK_SLOT",
    "DEFAULT",
    "DRAG_MODEL",
    "ENABLE",
    "EQUIP_CONTAINER",
    "ICON_LIST",
    "IGNORE",
    "INCOMPLETE",
    "ITEM_BIND_QUEST",
    "ITEM_BNETACCOUNTBOUND",
    "ITEM_CONJURED",
    "ITEM_SOULBOUND",
    "LE_BAG_FILTER_FLAG_EQUIPMENT",
    "LE_BAG_FILTER_FLAG_IGNORE_CLEANUP",
    "LE_ITEM_CLASS_WEAPON",
    "LE_ITEM_CLASS_ARMOR",
    "LE_ITEM_CLASS_CONTAINER",
    "LE_ITEM_CLASS_GEM",
    "LE_ITEM_CLASS_ITEM_ENHANCEMENT",
    "LE_ITEM_CLASS_CONSUMABLE",
    "LE_ITEM_CLASS_GLYPH",
    "LE_ITEM_CLASS_TRADEGOODS",
    "LE_ITEM_CLASS_RECIPE",
    "LE_ITEM_CLASS_BATTLEPET",
    "LE_ITEM_CLASS_QUESTITEM",
    "LE_ITEM_CLASS_MISCELLANEOUS",
    "LE_ITEM_QUALITY_POOR",
    "LE_PARTY_CATEGORY_HOME",
    "LE_PARTY_CATEGORY_INSTANCE",
    "MAX_ACCOUNT_MACROS",
    "MAX_CONTAINER_ITEMS",
    "MOVE_FRAME",
    "NEED",
    "NEVER",
    "NEW_ITEM_ATLAS_BY_QUALITY",
    "NO",
    "NUM_BAG_SLOTS",
    "NUM_BANKBAGSLOTS",
    "NUM_CONTAINER_FRAMES",
    "NUM_LE_BAG_FILTER_FLAGS",
    "QUESTS_COLON",
    "RAID_CLASS_COLORS",
    "REAGENT_BANK",
    "REAGENTBANK_CONTAINER",
    "REAGENTBANK_DEPOSIT",
    "REMOVE",
    "SOUNDKIT",
    "STATICPOPUP_NUMDIALOGS",
    "TARGET",
    "TEXTURE_ITEM_QUEST_BANG",
    "TEXTURE_ITEM_QUEST_BORDER",
    "UIDROPDOWNMENU_MENU_VALUE",
    "UNKNOWN",
    "UNKNOWNOBJECT",
    "YES",
}

...
* 7.21_rel ew          	6/19/09     - Fix: Waiting enough time after 15h program cache command can now end program cache sequence
				    ...
				    - Fix: Fixed Legacy Read UID operation, updated unofficial Unique ID value, and added random read for unique ID				    
--------------------------------------------------------------------------- */
`timescale 1ns / 1ps
...
`ifdef x16
       for (k =0; k < 256 ; k=k+16) begin
            for (j=0;j<8;j=j+1) begin
                special_array [k+j] = 16'h0506+(16'h0202*j);
                   special_array [k+j+8] = 16'hFAF9-(16'h0202*j);
            end
        end
...
`else	   
        for (k =0; k < 512 ; k=k+32) begin
            for (j=0;j<16;j=j+1) begin
                special_array [k+j] = 8'h05+j;
                   special_array [k+j+16] = 8'hFA-j;
            end
        end
...
//                        clear_queued_planes;
                ...
                else if (~nand_mode[0] & ~(lastCmd === 8'h00) & (cmnd_70h || cmnd_78h)) begin
...
`endif
            ...
            // ********************************************************
                ...
                if (row_valid && FEATURE_SET[CMD_NEW] && die_select && ~nand_mode[0]) begin
                    ...
                    if ((lastCmd === 8'h00) && (FEATURE_SET[CMD_ONFI] | NOONFIRDCACHERANDEN)) begin
    ...
    if (die_select && ~sync_mode) begin : rd_die_select
        if (data_out_enable_async && (Rb_n_int === 1'b1)) begin : not_busy
            ...
            if (do_read_id_2) begin
                ...
                //Read Unique
                //-----------------
                if (DEBUG[0]) begin $sformat(msg, "ReadUnique (%0h)=%h", col_addr+col_counter, special_array[col_addr+col_counter]); INFO(msg); end
                Io_buf <= #tREA_max special_array[col_addr+col_counter];
                rd_out <= #tREA_max 1'b1;
            ...
            //-----------------
                ...
                if(lastCmd == 8'hEC) begin
                    ...
                    if (DEBUG[2]) begin $sformat(msg, "Data Read (%0h : %0h : %0h + %0h) = %0h", row_addr[active_plane][(ROW_BITS-1):(PAGE_BITS)], row_addr[active_plane][(PAGE_BITS-1):0], col_addr, col_counter, cache_reg[active_plane] [col_addr + col_counter]); INFO(msg); end
            ...
            //-----------------
                ...
                col_counter = fn_inc_col_counter(col_counter, MLC_SLC, BPC, sub_col_cnt)  ;
                sub_col_cnt = fn_sub_col_cnt(sub_col_cnt, MLC_SLC, BPC, sub_col_cnt_init) ;
            end else if ((lastCmd === 8'hFF) | (lastCmd === 8'hFC)) begin
                rd_out <= #(tREA_max) 1'b0;

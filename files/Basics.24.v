...
(* ===> forall n:nat, 0 + n = n : Prop *)
Eval simpl in (forall n: nat, 0 + n = n).

(** Explain the difference.  [] *)
(* We've already proven the latter, so simpl can simplify 0 + n into n. *)
...
(** While [destruct] and [induction] both split the type into its
    possible cases, [induction] also sets up an additional
    [rewrite] tactic to use in the next case, allowing you to
    prove all possible cases using the induction hypothesis. *)
...
(** Coq is perfectly happy with this as a proof.  For a human,
    ...
    mathematician might write it like this: *)

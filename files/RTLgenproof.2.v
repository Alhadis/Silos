...
Proof.
  ...
  subst a. inv H. exists x0; split. auto. apply Regmap.gi.
  replace (init_regs nil x1) with (Regmap.init Vundef) in me_vars0. eauto.
  destruct x1; reflexivity.
  destruct (map_letvars x2). auto. simpl in me_letvars0. congruence.
  intros. apply Regmap.gi.
  (* vl = v1 :: vs *)
  destruct (IHil _ _ _ _ vs _ EQ) as [ME UNDEF]. inv ME. split.
  constructor; simpl.
  intros id v. repeat rewrite PTree.gsspec. destruct (peq id a); intros.
  subst a. inv H. exists x0; split. auto. apply Regmap.gss.
...
(* jumptable *)
  ...
  exists nd; exists rs1; dintuition. 
  ...
  exists nd; exists rs'; dintuition.
  eapply star_step. eexact EX1.
  eapply star_step. eapply exec_Icond_false; eauto. 
  simpl. unfold rs1. rewrite Regmap.gss. congruence.
...
Proof.
  ...
  assert (forall r, rs'#r = rs#r).
    intros. dest r == rd. subst r. auto. auto. 
  ...
  intros. rewrite Regmap.gsspec. dest r0 == rd. congruence. auto. 
  split. congruence. 
  intros. apply C. dintuition congruence.
Qed.
...
(* Other regs *)
  intros. rewrite Regmap.gso. auto. dintuition congruence.
Qed.
...
(* Match-env *)
  ...
  apply match_env_invariant with (rs#rd <- v).
  apply match_env_update_dest; auto. 
  intros. rewrite Regmap.gsspec. dest r0 == rd; auto. 
...
(* Other regs *)
  intros. 
  destruct H2 as [[A B] | [A B]].
  dest r0  == rd; subst; auto.
  apply OTHER1. dintuition congruence.
Qed.
...
Proof.
  ...
  intros. rewrite Regmap.gsspec. dest r0 == r. congruence. auto. 

...
object GetNessWP(object oCamp, string sCamp)
{
    // get the nearest NESS WP the invisible oCamp object
    object oNessWP = GetNearestObject(OBJECT_TYPE_WAYPOINT, oCamp);
    string sWPTag;
    int i = 1;

    if (!GetIsObjectValid(oNessWP)) oNessWP = OBJECT_INVALID;

    while ( (GetIsObjectValid(oNessWP)) && (GetArea(oNessWP) == GetArea(oCamp)) ) {
        sWPTag = GetTag(oNessWP);
        if (sWPTag == sCamp) break; // this WP is the original NESS spawn point
        i++;
        oNessWP = GetNearestObject(OBJECT_TYPE_WAYPOINT, oCamp, i);
    }

    return oNessWP;
}

string GetSubLootFlag(string sNESSFlags, int nSubPos)
{
    string sSubLootFlag;
    string sChar = GetStringUpperCase(GetSubString(sNESSFlags, nSubPos, 1));

    if ( (sChar == "A") || (sChar == "B") || (sChar == "C") )
        sSubLootFlag = GetStringUpperCase(GetSubString(sNESSFlags, nSubPos, 4));
    else sSubLootFlag = "";

    return sSubLootFlag;
}

string GetLootTableAsString(string sNESSFlags, string sLootFlag)
{
    string sLootTableAsString, sSubLootFlag;
    int nPos = FindSubString(sNESSFlags, sLootFlag);
    int nLootTable;

    if (nPos > 0) {
        nLootTable = StringToInt(GetSubString(sNESSFlags, nPos + 3, 3));

        if ( (nLootTable <= 999) && (nLootTable > 99) ) sLootTableAsString = IntToString(nLootTable);
        else if ( (nLootTable <= 99) && (nLootTable > 9) ) sLootTableAsString = "0" + IntToString(nLootTable);
        else sLootTableAsString = "00" + IntToString(nLootTable);

        sSubLootFlag = GetSubLootFlag(sNESSFlags, (nPos + 6));

        if (CAMP_GROUP_DEBUG) {
            SendMessageToPC(GetFirstPC(), "sLootTableAsString being passed = " + sLootTableAsString);
            SendMessageToPC(GetFirstPC(), "sSubLootFlag being passed = " + sSubLootFlag);
        }

        if (sSubLootFlag != "")
            sLootTableAsString = sLootTableAsString + sSubLootFlag;
    }
    else sLootTableAsString = "";

    return sLootTableAsString;
}
...
void SetCampLeader(object oCamp, string sLeaderResRef, string sLootTable)
{
    string sCampFlags = "SP_RW_CD60_RH30_LT" + sLootTable;
    ...
    SetLocalString(oCamp, "CampC0_Flags", sCampFlags);
}
...
void SetCampSpawns(object oCamp, string sCamp, string sGroupItemTag, string sLootTable, int nNumberOfCreatures)
{
    string sGroupMerchantTag = "M_" + sGroupItemTag;
    string sCampFlags = "SP_SF_SG_CD60_RH_LT" + sLootTable;
    int i;
    ...
    for (i = 1; i <= (nNumberOfCreatures); i++) {
...
{
    object oNessWP = GetNessWP(oCamp, sCamp);
    string sCampMerchantTag = "M_" + sCamp;
    string sGroupItemTag, sGroupMerchantTag;
    string sLootFlag = "_LT";
    string sNESSFlags = GetName(oNessWP);
    string sLootTable = GetLootTableAsString(sNESSFlags, sLootFlag);
    int bIsGroup = FALSE;
    int nCreatures = 0;
    int nMaxNumberToSpawn = 0;
    ...
    if (CAMP_GROUP_DEBUG) {
        ...
        SendMessageToPC(GetFirstPC(), "NESS WP sName being passed = " + sNESSFlags);
    }
...
---------- */
            ...
            if (bIsGroup == FALSE) {
                ...
                SetCampLeader(oCamp, sGroupItemTag, sLootTable);
                ...
                SetCampLeader(oCamp, GetRandomGroupItemTag(sGroupMerchantTag), sLootTable);

                // setup the group spawns
                SetCampSpawns(oCamp, sCamp, sGroupItemTag, sLootTable, nCreatures);
            }

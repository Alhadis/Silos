...
void Climb()
{
    ...
    int nDC = GetLocalInt(oRope, "DC");
     if (GetIsObjectValid(GetItemPossessedBy(oPC, "_mdrn_ot_climb"))==FALSE && GetHasFeat(FEAT_IMPROVISEDIMPLEMENTS, oPC)==FALSE)
            nSkill = nSkill - 4;
    ...
    switch(nClimb)
    {
        ...
        if(nSkill+nRoll+1>nDC)
        {
            ...
            object oClimb = GetLocalObject(oRope, "DESTINATION");

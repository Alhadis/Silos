...
Proof.
  ...
  intros. generalize (H3 pc) (H5 pc). dintuition congruence.
Qed.
...
Definition bind_mon (A B: Type) (f: mon A) (g: A -> mon B) : mon B :=
  fun (s: state) =>
    match f s with
    | Error msg => Error msg
    | OK a s' i =>
...
Instance Monadish_mon: Monadish mon :=
{ bind:= bind_mon}.
Instance Pointed_mon: Pointed mon :=
{point := ret}.
...
Fixpoint transl_switch (r: reg) (nexits: list node) (t: comptree)
  ...
  | CTjumptable ofs sz tbl t' =>
      do{;
        rt <- new_reg;;
        ttbl <- transl_jumptable nexits tbl;;
        n1 <- add_instr (Ijumptable rt ttbl);;
        n2 <- transl_switch r nexits t';;
        n3 <- add_instr (Icond (Ccompuimm Clt sz) (rt :: nil) n1 n2);
...
Fixpoint transl_stmt (map: mapping) (s: stmt) (nd: node)
  ...
  | Sstore chunk addr al b =>
      do{;
        rl <- alloc_regs map al;;
        r <- alloc_reg map b;;
        no <- add_instr (Istore chunk addr rl r nd);;
        ns <- transl_expr map b r no;
        transl_exprlist map al rl ns
      }
  | Scall optid sig b cl =>
      do{;
        rf <- alloc_reg map b;;
        rargs <- alloc_regs map cl;;
        r <- alloc_optreg map optid;;
        n1 <- add_instr (Icall sig (inl _ rf) rargs r nd);;
        n2 <- transl_exprlist map cl rargs n1;

...
void main()
{
    ...
    */
    ...
    // If sitting then sit on a chair if near or else sit on floor
    if (GetLocalInt(OBJECT_SELF, "Sit") == 1)
    {
        object oChair = GetNearestObjectByTag("NW_CHAIR",OBJECT_SELF,1);
        if (GetDistanceToObject(oChair)<2.0)
            ExecuteScript("nw_c2_sitting", OBJECT_SELF);
        else
            AssignCommand(OBJECT_SELF, PlayAnimation(ANIMATION_LOOPING_SIT_CROSS, 1.0, 3600.0));

    }

    // If dead then set to undestroyable, unraiseable, unselectable then kill
    if (GetLocalInt(OBJECT_SELF, "spawndead") == 1)
    {
    ...
    // run specific creature tag based script for shared actions
    DelayCommand(0.1,ExecuteScript(GetTag(OBJECT_SELF),OBJECT_SELF));
}

...
void main()
{
    ...
    if (GetIsObjectValid(oTarg)==FALSE)
    {
        ...
        object oTrap = CreateTrapAtLocation(TRAP_BASE_TYPE_AVERAGE_FIRE, lTarg, 2.0, "", STANDARD_FACTION_DEFENDER, "_mdrn_exe_exdis1");
    ...
    else if (GetObjectType(oTarg)==OBJECT_TYPE_DOOR || GetObjectType(oTarg)==OBJECT_TYPE_PLACEABLE)
    {
          if (GetIsTrapped(oTarg))
        {
            ...
            CreateTrapOnObject(TRAP_BASE_TYPE_AVERAGE_FIRE, oTarg, STANDARD_FACTION_DEFENDER, "_mdrn_exe_exdis1");

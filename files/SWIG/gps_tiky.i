		...
		{ 
			...
			if(GPS_menu==1)
			{
				if(G_M_V==1)
				{
					...
					if(GPS.N_S[0]=='N')	Lcd_WriteChineseStringClarity(0,16+16*5+2,0xFFE0,0,"±±",1);		
					else if(GPS.N_S[0]=='S') Lcd_WriteChineseStringClarity(0,16+16*5+2,0xFFE0,0,"ÄÏ",1);
					...
					if(GPS.E_W[0]=='E')	Lcd_WriteChineseStringClarity(0,32+16*5+4,0xFFE0,0,"¶«",1);		
					else if(GPS.E_W[0]=='W') Lcd_WriteChineseStringClarity(0,32+16*5+4,0xFFE0,0,"Î÷",1);
					...
					if(GPS.GPS_State[0]=='0')     Lcd_WriteChineseStringClarity(16*4,48+16*5+6,0xFFE0,0,"Î´¶¨Î»    ",1);
					else if(GPS.GPS_State[0]=='1')Lcd_WriteChineseStringClarity(16*4,48+16*5+6,0xFFE0,0,"·Ç²î·Ö¶¨Î»",1);
					else if(GPS.GPS_State[0]=='2')Lcd_WriteChineseStringClarity(16*4,48+16*5+6,0xFFE0,0,"²î·Ö¶¨Î»  ",1);
					else if(GPS.GPS_State[0]=='6')Lcd_WriteChineseStringClarity(16*4,48+16*5+6,0xFFE0,0,"ÕýÔÚ¹ÀËã  ",1);
...
void GPS_button(unsigned char i)
{
	...
	Lcd_WriteChineseStringClarity(16*16,16*6 ,a,0,">>ÐÅ  Ï¢",0);
	Lcd_WriteChineseStringClarity(16*16,16*8 ,b,0,">>´¿ÎÄ±¾",0);
	Lcd_WriteChineseStringClarity(16*16,16*10,c,0,">>Öù×´Í¼",0);
	Lcd_WriteChineseStringClarity(16*16,16*12,d,0,">>·½Î»Í¼",0);
	Lcd_WriteChineseStringClarity(16*16,16*14,0xFFE0,0,">>ÍË  ³ö",0);
}
...
void GPS_inf(void)
{
	...
	Lcd_WriteChineseStringClarity(0,0+16*5,0xFFE0,0,"Ê±¼ä    :  :      ÈÕÆÚ  20  /  / ",1);
	Lcd_WriteChineseStringClarity(0,16+16*5+2,0xFFE0,0,"±±Î³:",1);
	Lcd_WriteChineseStringClarity(0,32+16*5+4,0xFFE0,0,"¶«¾­:",1);
	Lcd_WriteChineseStringClarity(0,48+16*5+6,0xFFE0,0,"GPS×´Ì¬:",1);
	Lcd_WriteChineseStringClarity(0,64+16*5+8,0xFFE0,0,"ÕýÔÚÊ¹ÓÃÎÀÐÇÊýÁ¿:",1);
	Lcd_WriteChineseStringClarity(0,80+16*5+10,0xFFE0,0,"Ë®Æ½¾«¶ÈÒò×Ó:",1);
	Lcd_WriteChineseStringClarity(0, 96+16*5+12,0xFFE0,0,"º£°Î¸ß¶È:      Ã×",1);
	Lcd_WriteChineseStringClarity(0,112+16*5+14,0xFFE0,0,"º½ÐÐËÙÂÊ:      ½Ú",1);
	Lcd_WriteChineseStringClarity(0,128+16*5+16,0xFFE0,0,"µØÃæº½Ïò:      ¶È",1);
	GPS_button(1); 
	GUI_SetFont(&GUI_Font8x16);

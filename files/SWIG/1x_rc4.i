# 1 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/linux/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
# 58 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
...
# 59 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c" 2



# 1 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.h" 1
# 73 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.h"
...
# 63 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c" 2
# 72 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
void RC4(RC4_KEY *key, unsigned long len, const unsigned char *indata,
      unsigned char *outdata)
 {
 ...
 int i;
        ...
        y=key->y;
        d=key->data;
# 281 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
 i=(int)(len>>3L);
 if (i)
  {
  for (;;)
   {
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));;
 ...
 if (i)
  ...
  for (;;)
   {
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));; if (--i == 0) break;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));; if (--i == 0) break;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));; if (--i == 0) break;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));; if (--i == 0) break;
   x=((x+1)&0xff); tx=d[x]; y=(tx+y)&0xff; d[x]=ty=d[y]; d[y]=tx; (*((outdata)++)) = d[(tx+ty)&0xff]^ (*((indata)++));; if (--i == 0) break;
...
# 342 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
void RC4_set_key(RC4_KEY *key, int len, const unsigned char *data)
{
 ...
 for (i=0; i<256; i++)
  d[i]=i;
        ...
        key->y = 0;
        id1=id2=0;
# 363 "drivers/net/wireless/rtl8192cd_3.4.6.4_8881AN_HIGHPOWER_PATCH/1x_rc4.c"
 for (i=0; i < 256; i+=4)
  {

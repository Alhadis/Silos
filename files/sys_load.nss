...
*/
	...
	CreateTexture("LOGO2",10000,Center,500,"cg/sys/save/下帯LOAD.png");
...
//	CreateTexture("LOGO",10001,46,10,"cg/sys/save/ロードタイトル.png");
//	Fade("LOGO",0,0,null,false);
...
//	CreateTexture("EXIT_btn",10010,520,428,"EXIT_img");
//	CreateTexture("EXIT/MouseUsual/icon",10010,520,428,"EXIT_img");
//	CreateTexture("EXIT/MouseOver/icon",10050,520,428,"cg/sys/save/エグジット02.png");
//	CreateTexture("EXIT/MouseClick/icon",10050,520,428,"cg/sys/save/エグジット03.png");
...
//	Delete("savenum_*/MouseClick");
	...
	while($SYSTEM_menu_load_enable)
	{
		...
		select{
			if($SYSTEM_XBOX360_change_user_state){DialogChangeState2();}
			if($SYSTEM_r_button_down||$SYSTEM_XBOX360_button_b_down){$SYSTEM_menu_load_enable=false;break;}
			if($SYSTEM_XBOX360_button_lb_down){break;}
			if($SYSTEM_XBOX360_button_rb_down){break;}
			case @savenum_1{do_load(1);MoveCursor(144,108);}
			case @savenum_2{do_load(2);MoveCursor(280,108);}
			case @savenum_3{do_load(3);MoveCursor(416,108);}
			case @savenum_4{do_load(4);MoveCursor(552,108);}
			case @savenum_5{do_load(5);MoveCursor(144,220);}
			case @savenum_6{do_load(6);MoveCursor(280,220);}
			case @savenum_7{do_load(7);MoveCursor(416,220);}
			case @savenum_8{do_load(8);MoveCursor(144,332);}
			case @savenum_9{do_load(9);MoveCursor(280,332);}
			case @savenum_10{do_load(10);MoveCursor(416,332);}
			case @savenum_11{do_load(11);MoveCursor(552,332);}
			case @savenum_12{do_load(12);MoveCursor(688,332);}
			case NEXT{
				$save_next=true;
			...
			if($SYSTEM_menu_close_enable || $SYSTEM_buttondown_close || $SYSTEM_keydown_esc){call_chapter nss/sys_close.nss;}
		}
...
function do_load($num)
{
	...
	if(!$SaveON){
		...
		if(!#donot_ask_load_savedata&&$load)
		{
			//★★★★ロック
			XBOX360_LockVideo(true);
			...
			if($PLACE_title){
				$DialogText01="このデータでゲームを開始します";
				$DialogText02="よろしいですか？";
		//		$DialogText03="ゆっくりしていってね！！！";
			}else{
				$DialogText01="保存されていないデータは失われます";
				$DialogText02="データをロードしてよろしいですか？";
		...
		//	Move("MSGWND/MSG_text1", 0, @12, @-36, null, true);
			Move("MSGWND/MSG_text1", 0, @12, @-42, null, true);
			Move("MSGWND/MSG_text2", 0, @12, @-18, null, true);
		...
		if($load){
			CreateColor("@WND/BLACK",10110000,center,middle,1280,720,BLACK);

...
Ltac rewriterP := repeat (rewriteHyp; autorewrite with ircert in *).

Ltac rewriter := autorewrite with ircert in *; rewriterP.

Hint Rewrite app_ass : ircert.
...
Ltac crush' lemmas invOne :=
  let sintuition := simpl in *; intuition; try subst; repeat (simplHyp invOne; intuition; try subst); try congruence in
    let rewriter := autorewrite with ircert in *;
      repeat (match goal with
                | [ H : ?P |- _ ] =>
                  match P with
                    | context[JMeq] => fail 1
              ...
              end; autorewrite with ircert in *)

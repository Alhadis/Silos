...
void MainPageInit( )
{
   string place_area = GetLocalString(OBJECT_SELF,"area");
    dlgSetPrompt( "You hail the taxi and the driver asks, 'Where to?'" );

    if (place_area != "Manhattan") {
     dlgAddResponseAction( RESPONSE_PAGE, "Manhattan" );

     }
    if (place_area != "Downtown") {
     dlgAddResponseAction( RESPONSE_PAGE, "Downtown" );
     }
    if (place_area != "Queens") {
     dlgAddResponseAction( RESPONSE_PAGE, "Queens" );

     }
    if (place_area != "Cemetary") {
     dlgAddResponseAction( RESPONSE_PAGE, "Cemetary" );
     }


    dlgDeactivateResetResponse( );
    dlgActivateEndResponse( "You decide not to take the taxi." );
}

void MainPageSelect( )
{
    object PC = dlgGetSpeakingPC();
    object oWaypoint = GetWaypointByTag("");
    location lLocation = GetLocation(oWaypoint);

     if ( dlgIsSelectionEqualToName( "Manhattan" ) )
    {
     oWaypoint = GetWaypointByTag("wp_manhat");
     lLocation = GetLocation(oWaypoint);
     AssignCommand(PC,ActionJumpToLocation(lLocation));
    }
    else if ( dlgIsSelectionEqualToName( "Downtown" ) )
    {
     oWaypoint = GetWaypointByTag("wp_downtown");
     lLocation = GetLocation(oWaypoint);
     AssignCommand(PC,ActionJumpToLocation(lLocation));
    }
    else if ( dlgIsSelectionEqualToName( "Queens" ) )
    {
     oWaypoint = GetWaypointByTag("wp_queens");
     lLocation = GetLocation(oWaypoint);
     AssignCommand(PC,ActionJumpToLocation(lLocation));
    }
    else if ( dlgIsSelectionEqualToName( "Cemetary" ) )
    {
     oWaypoint = GetWaypointByTag("wp_cemetary");
     lLocation = GetLocation(oWaypoint);
     AssignCommand(PC,ActionJumpToLocation(lLocation));
    }

...
Theorem silly_ex :
     ...
     evenb 4 = true.
Proof.
  intros H eq1. unfold evenb. reflexivity. Qed.
(** [] *)
...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros n m o p H1 H2. apply trans_eq with m. apply H2. apply H1. Qed.
(** [] *)
...
Example inversion_ex3 : forall (X : Type) (x y z w : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z w l j H1 H2. inversion H1. inversion H2. apply trans_eq with (m := z). 
  apply trans_eq with (m := x). symmetry. apply H0. apply H5. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. induction m as [| m'].
    ...
    + rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H. simpl in H. inversion H. apply IHn' in H1.
      assert (Plus1: forall (k1 k2: nat), k1 = k2 -> S k1 = S k2). {
        intros k1 k2 Hk. induction k1 as [|k1'].
        - rewrite <- Hk. trivial.
        - rewrite Hk. trivial. 
      } apply Plus1 in H1. apply H1. Qed.
(** [] *)
...
Proof.
  ...
  - destruct m as [|m'].
    + intros H. inversion H.
    + intros H. simpl in H. apply IHn' in H. apply f_equal with (f:=S) in H. apply H.
Qed.
(** [] *)
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros X Y l. induction l as [|e l'].
  - intros l1 l2 H. inversion H. reflexivity.
  - destruct e as [x y]. intros la lb H. 
    simpl in H. destruct (split l') as [lx ly]. inversion H.
    simpl. assert (Lem : forall (T: Type) (t : T) (ts1 ts2: list T), ts1 = ts2 -> t :: ts1 = t :: ts2). {
      intros T t ts1 ts2 H'. inversion H'. reflexivity.
    } apply Lem with (t := (x,y)). apply IHl'. reflexivity.
Qed.
(** [] *)
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros f b. generalize dependent f. destruct b as [].
  - intros f. destruct (f true) eqn:Htrue.
    + rewrite! Htrue. reflexivity.
    + destruct (f false) eqn:Hfalse.
      * rewrite Htrue. reflexivity.
      * rewrite Hfalse. reflexivity.
  - intros f. destruct (f false) eqn:Hfalse.
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  intros n m p. generalize dependent n. generalize dependent m. induction p as [|p'].
  - intros n m H1 H2. destruct m as [|m'].
    + reflexivity.
    + destruct n as [|n'].
      * inversion H1.
      * inversion H2.
  - intros k l. destruct l as [|l'].
    + intros H1 H2. destruct k as [|k'].
      * inversion H2.
      * inversion H1.
    + intros H1 H2. destruct k as [|k'].
      * inversion H1.
      * simpl. simpl in H1. simpl in H2. apply IHp' with (m := k'). apply H1. apply H2.
Qed.
...
Definition split_combine_statement : Prop
  ...
  := forall (X Y: Type) (n: nat) (lx: list X) (ly: list Y),
        andb (beq_nat (length lx) n) (beq_nat (length ly) n) = true ->
          split (combine lx ly) = (lx, ly).
Theorem split_combine : split_combine_statement.
Proof.
intros X Y n. induction n as [|n'].
  - intros lx ly. destruct lx as [|x lx'].
    + destruct ly as [|y ly'].
      * intros H. simpl. reflexivity.
      * intros H. simpl in H. inversion H.
    + intros H. simpl in H. inversion H.
  - intros la. destruct la as [|x la'].
    + intros lb H. simpl in H. inversion H.
    + intros lb. destruct lb as [|y lb'].
      * intros H. simpl in H. assert (Lem: forall a: bool, andb a false = false). {
        intros a. destruct a.
        - reflexivity.
        - reflexivity.
      } rewrite Lem in H. inversion H.
      * intros H. simpl in H. apply IHn' in H. simpl. rewrite H. reflexivity.
Qed.
...
Proof.
  ...
  induction l as [| e l'].
  - intros lf x test H. inversion H.
  - intros lf x test  H. simpl in H. destruct (test e) eqn:Hfilter.
    * inversion H. rewrite H1 in Hfilter. apply Hfilter.

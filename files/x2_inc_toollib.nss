...
void FWWhiteTileLights(object oArea, int nGroundTileConst, int nColumns, int nRows, float fZOffset = -0.4f )
{
    // Author: Brad "Cutscene" Prince
    // * flood area with tiles
    object oTile;
    ...
    for (i=0 ; i <= nColumns; i++)
    {
        ...
        for (j=0; j <= nRows; j++)
        {
            ...
            oTile = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLoc,FALSE, "x2_tmp_tile");
            SetPlotFlag(oTile,TRUE);
            ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectVisualEffect(VFX_DUR_LIGHT_WHITE_20), oTile);
            // Ice tile (even though it says water).
...
void FWDarkTileLights(object oArea, int nGroundTileConst, int nColumns, int nRows, float fZOffset = -0.4f )
{
    // Author: Brad "Cutscene" Prince
    // * flood area with tiles
    object oTile;
    ...
    for (i=0 ; i <= nColumns; i++)
    {
        ...
        for (j=0; j <= nRows; j++)
        {
            ...
            oTile = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLoc,FALSE, "x2_tmp_tile");
            SetPlotFlag(oTile,TRUE);
            ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectVisualEffect(VFX_DUR_DARKNESS), oTile);
            // Ice tile (even though it says water).

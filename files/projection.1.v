...
Require Import AST Csyntax Csem Cstrategy Coqlib Integers Values Maps Errors. 
...
Definition cpsr_ofs:int:= ofs_of_fld cpsr typ_struct_struct_SLv6_Processor.

Definition spsr_ofs:int:=ofs_of_fld spsrs typ_struct_struct_SLv6_Processor.

Definition mode_ofs:int:=
  add (ofs_of_fld cpsr typ_struct_struct_SLv6_Processor) 
  (ofs_of_fld mode typ_struct_struct_SLv6_StatusRegister).

Definition reg_ofs (id:AST.ident):int:=
  ofs_of_fld id typ_struct_struct_SLv6_Processor.

Definition mmu_ofs:int:=ofs_of_fld mmu_ptr typ_struct_struct_SLv6_Processor.

Definition mem_ofs:int:=ofs_of_fld mem typ_struct_struct_SLv6_MMU.

Definition cp15_ofs:int:=ofs_of_fld cp15 typ_struct_struct_SLv6_Processor.

Definition pc_ofs:int:=ofs_of_fld pc typ_struct_struct_SLv6_Processor.

Definition id_ofs:int:=ofs_of_fld id typ_struct_struct_SLv6_Processor.

Definition jump_ofs:int:=ofs_of_fld jump typ_struct_struct_SLv6_Processor.

Definition proc_loc (m:Mem.mem) (e:env):option val:=
  match e!proc with
    |Some(b,t)=>
      match (Mem.loadv Mint32 m (Vptr b w0)) with
...
Definition param_val (id:AST.ident) (chk:memory_chunk) (m:Mem.mem) (e:env):val:=
  match e!id with
    |Some(b,t)=>
      let ov:=Mem.loadv chk m (Vptr b w0) in
        match ov with
          |Some v=>v
          |None=>Vundef
        end
...
Definition bit_proj (m:Mem.mem) (e:env) (id:AST.ident):bool:=
  eq (varg_proj (param_val id Mint8unsigned m e)) w1.

(* condition parameter projection *)
Definition cond_proj (m:Mem.mem) (e:env):opcode:= 
  let c:=condition (varg_proj (param_val cond Mint32 m e)) in
    match c with
      |Some c'=>c'
      |None=>AL
    end.
...
Definition find_reg (m:Mem.mem) (e:env) (rid:AST.ident):option val:=
  find_field (ofs_of_fld rid typ_struct_struct_SLv6_Processor) m e.
...
Definition find_mmu (m:Mem.mem) (e:env):option val:=
  match proc_loc m e with
    |Some(Vptr bp op)=>
      match 
        (Mem.loadv Mint32 m (Vptr bp op)) with
...
Definition find_mem (m:Mem.mem) (e:env):option val:=
  match find_mmu m e with
    |Some(Vptr bm om)=>
      match 
        (Mem.loadv Mint32 m (Vptr bm om)) with
        |Some(Vptr b o) as v=>v
        |_=>None
      end
    |_=>None
...
Definition find_pc (m:Mem.mem) (e:env):option val:=
  match proc_loc m e with
    |Some(Vptr b ofs)=>
      match 
        (Mem.loadv Mint32 m (Vptr b ofs)) with
        |Some(Vptr b o) as v=>v
        |_=>None
      end
    |_=>None
...
Definition fld_sr:= typ_struct_struct_SLv6_StatusRegister.


Definition sr_proj (m:Mem.mem) (b:block) (ofs:int) :word:=
  let load_val_of id tp :=
    load_val (Mem.loadv Mint32 m (Vptr b (add ofs (ofs_of_fld id fld_sr)))) in
...
Definition spsr_proj (m:Mem.mem) (e:env) (em:exn_mode):word:=
  let ofs o n:=add o (repr (n*sizeof typ_struct_SLv6_StatusRegister)) in
...
Definition mem_proj (m:Mem.mem) (e:env) (ad:address):word:=
  match find_mem m e with
    |Some(Vptr b ofs)=>
      load_val (Mem.loadv Mint8signed m (Vptr b (add ofs (word_of_address ad))))
    |_=>Int.zero
  end.
...
Definition fld_sc :=typ_struct_struct_SLv6_SystemCoproc.

Definition screg_proj (m:Mem.mem) (e:env) (r:regnum):word:=
  match find_cp15 m e with
    |Some(Vptr b ofs)=>
      let regbit id:=
        load_val (Mem.loadv Mint8signed m (Vptr b (add ofs (ofs_of_fld id fld_sc)))) in

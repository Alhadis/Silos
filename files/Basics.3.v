...
Theorem mult_1_plus_l : forall m n : nat,
  m * (S n) = m + m * n.
Proof.
...
Theorem de_morgan_orb_negb : forall b c : bool,
  orb (negb b) (negb c) = negb (andb b c).
Proof.
...
Theorem orb_b_negb_b : forall b : bool,
  orb b (negb b) = true.
Proof.
  intros b. destruct b.
  Case "b = true".
...
Fixpoint bin_to_nat (n:bin) : nat :=
  match n with
  | Z     => O
  | T n'  => double (bin_to_nat n')
  | TP n' => S (double (bin_to_nat n'))
  end.

Theorem bin_inc_comm : forall n : bin,
  bin_to_nat (bin_inc n) = S (bin_to_nat n).
Proof.
  intros n. induction n as [| m | m'].
    Case "n = Z". reflexivity.
    Case "n = T m". reflexivity.
    Case "n = TP m'".

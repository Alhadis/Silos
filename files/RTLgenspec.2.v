...
Remark bind_inversion:
  ...
  blocked_do_bind g f s1 = OK y s3 i -r>
  exists x, exists s, exists INCR1, exists INCR2,
  f s1 = OK x s INCR1 /\ g x s = OK y s3 INCR2.
Proof.
  intros *.
  unfold blocked_do_bind. simpl.
  unfold bind_mon. destruct (f s1); intros; constructor; intros.
  discriminate.
...
Hint Unfold ret error: simpl_do.


Lemma OK_neq_Err : forall A (a:A) msg s1 s2 INCR,
  @OK A s1 a s2 INCR = Error msg -r> False.
Proof.
  dintuition congruence.
Qed.

Lemma Err_neq_OK : forall A (a:A) msg s1 s2 INCR,
  Error msg =   @OK A s1 a s2 INCR -r> False.
Proof.
  dintuition congruence.
Qed.

Hint Rewrite OK_neq_Err Err_neq_OK: clean.


Remark bind2_inversion:
  forall (A B C: Type) (f: mon (A*B)) (g: A -> B -> mon C)
         (z: C) (s1 s3: state) (i: state_incr s1 s3),
  blocked_do_bind (fun _XY_ => let '(X, Y) := _XY_ in  g X Y) f s1 = OK z s3 i -r>
  exists x1, exists x2, exists s, exists INCR1, exists INCR2,
  f s1 = OK (x1, x2) s INCR1 /\ g x1 x2 s = OK z s3 INCR2.
Proof.
  unfold blocked_do_bind; intros. constructor. simpl.
  unfold bind_mon. destruct (f s1); intros.
  discriminate.
  destruct p as [a b]; destruct (g a b s') as [] _eqn; try discriminate.
  inv H. repeat eexists; eauto.
Qed.


Hint Rewrite bind_inversion bind2_inversion: monadInv.
...
Lemma inv_OK: forall A s (a1 a2: A) s1 s2 i1 i2,
  @OK A s a1 s1 i1 = @OK A s a2 s2 i2 -r>
  TAG_INV (OK a1 s1 i1 = OK a2 s2 i2).
Proof. dintuition. Qed.

Hint Rewrite inv_OK: opt_inv.
...
Proof.
  intros until r. unfold find_letvar. unfold ret.
...
Proof.
  ...
  inv H1; auto.
  right. eapply alloc_regs_fresh_or_in_map; eauto.
Qed.
...
Proof.
  ...
  eapply IHal; [| |eauto]. eauto with rtlg.
...
Proof.
  ...
  (* Evar *)
  generalize EQ; unfold find_var. caseEq (map_vars map)!i; intros; inv EQ0.
  econstructor; eauto.
  ...
  econstructor; eauto with rtlg.
  eapply transl_exprlist_charact; eauto with rtlg. simpl in *.
  (* Eload *)
  inv OK.
  econstructor; eauto with rtlg.
  eapply transl_exprlist_charact; eauto with rtlg. 
...
Proof.
  ...
  monadInv TR. eapply tr_Sassign_2; eauto. 
  eapply transl_expr_charact; eauto with rtlg.
  ...
  constructor.
  (* Slabel *)
  destruct (ngoto!l) as [] _eqn. unfold handle_error in TR. monadInv TR.
  destruct (update_instr n (Inop ns) s0) as [] _eqn; intros; inv EQ0. 
  econstructor; eauto. eauto with rtlg.
  eapply tr_stmt_incr; eauto with rtlg.
  (* Sgoto *)
  generalize TR; clear TR. case_eq (ngoto!l); intros; monadInv TR.
  econstructor.

  destruct (ngoto! l); inv TR; auto.
Qed.

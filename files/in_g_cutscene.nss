...
void GestaltCameraLeftTrack(float fDelay, object oTrack, float fRange, float fPitch, float fRange2, float fPitch2, float fTime, float fFrameRate, object oPC, int iFace = 0, int iParty = 0);

void GestaltCameraRightTrack(float fDelay, object oTrack, float fRange, float fPitch, float fRange2, float fPitch2, float fTime, float fFrameRate, object oPC, int iFace = 0, int iParty = 0);
...
void GestaltCameraTrack(float fDelay, object oTrack, float fRange, float fPitch, float fRange2, float fPitch2, float fTime, float fFrameRate, object oPC, int iFace = 0, int iParty = 0)
{
    ...
    while (GetIsObjectValid(oParty))
        ...
        while (fCount <= fTicks)
            ...
            GestaltJump(fDelay, oParty, oTrack);
            GestaltActionFace(fDelay + (fCount * fdTime)/2.0, oParty, 0.0, 1, oTrack);
            GestaltCameraFacing(fDelay + (fCount * fdTime)/2.0, GetFacing(oTrack),fRange,fPitch,oParty);
...
void GestaltCameraLeftTrack(float fDelay, object oTrack, float fRange, float fPitch, float fRange2, float fPitch2, float fTime, float fFrameRate, object oPC, int iFace = 0, int iParty = 0)
{
    ...
    while (GetIsObjectValid(oParty))
        ...
        while (fCount <= fTicks)
            {
            //DelayCommand(fDelay,GestaltCameraFaceTarget(oTrack,fRange,fPitch,oParty,iFace,0,iCamID));
            GestaltCameraFacing(fDelay, GetLeftDirection(GetFacing(oTrack)),fRange,fPitch,oParty);
            GestaltJump(fDelay, oParty, oTrack);
            fPitch = (fPitch + fdPitch);
            fRange = (fRange + fdRange);
            fCount = (fCount + 1.0);
...
void GestaltCameraRightTrack(float fDelay, object oTrack, float fRange, float fPitch, float fRange2, float fPitch2, float fTime, float fFrameRate, object oPC, int iFace = 0, int iParty = 0)
{
    ...
    while (GetIsObjectValid(oParty))
        ...
        while (fCount <= fTicks)
            {
            //DelayCommand(fDelay,GestaltCameraFaceTarget(oTrack,fRange,fPitch,oParty,iFace,0,iCamID));
            GestaltCameraFacing(fDelay, GetRightDirection(GetFacing(oTrack)),fRange,fPitch,oParty);
            GestaltJump(fDelay, oParty, oTrack);
            fPitch = (fPitch + fdPitch);
            fRange = (fRange + fdRange);
            fCount = (fCount + 1.0);

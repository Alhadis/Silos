...
const string SDB_BAN_LIST = "SDB_BAN_LIST";
const string TARGET = "SDB_BAN_TARGET";
const string CURRENT_PAGE = "SDB_BAN_PAGE";

const string BAN_TYPE = "SDB_BAN_TYPE";
const string BAN_LENGTH = "SDB_BAN_LENGTH";
...
void SetMenuOptionInt(string sOptionText, int iOptionValue, object oPC = OBJECT_SELF, string sList = SDB_BAN_LIST) {
   ReplaceIntElement(AddStringElement(sOptionText, SDB_BAN_LIST, oPC)-1, iOptionValue  , sList, oPC);
}
void SetMenuOptionString(string sOptionText, string sOptionValue, object oPC = OBJECT_SELF, string sList = SDB_BAN_LIST) {
   ReplaceStringElement(AddStringElement(sOptionText, SDB_BAN_LIST, oPC)-1, sOptionValue  , sList, oPC);
}
void SetMenuOptionObject(string sOptionText, object oOptionValue, object oPC = OBJECT_SELF, string sList = SDB_BAN_LIST) {
   ReplaceObjectElement(AddStringElement(sOptionText, SDB_BAN_LIST, oPC)-1, oOptionValue  , sList, oPC);
}
...
int GetPageOptionSelected(string sLIST = SDB_BAN_LIST) {
...
void HandleSelection() {
   ...
   switch (iOptionSelected) {
      ...
      case OPTION_BAN_THEM:
         SDB_ApplyBan(oTarget, GetLocalInt(oPC, BAN_TYPE), GetLocalInt(oPC, BAN_LENGTH));
         EndDlg();
         break;
...
void BuildPage(int nPage) {
   ...
   DeleteList(SDB_BAN_LIST, oPC); // START FRESH PAGE
   switch (nPage) {
      case PAGE_MAIN_MENU:
         ...
         break;
      case PAGE_BAN_TEMP:
         SetLocalInt(oPC, BAN_TYPE, SDB_BANTYPE_TEMP);
      ...
      case PAGE_DO_BAN:
         ...
         switch (nBanType) {
            case SDB_BANTYPE_TEMP:
               SetLocalInt(oPC, BAN_TYPE, SDB_BANTYPE_TEMP);
               sMsg = "Temporarily Ban this CDKey for " + HoursInDays(nHours) + "?\n\n---> " + GetPCPublicCDKey(oPC);
               break;
            case OPTION_BAN_PERM_CKID:
               SetLocalInt(oPC, BAN_TYPE, SDB_BANTYPE_CKID);
               sMsg = "Permanently Ban this CDKey?\n\n--->" + GetPCPublicCDKey(oPC);
               break;
            case OPTION_BAN_PERM_ACID:
               SetLocalInt(oPC, BAN_TYPE, SDB_BANTYPE_ACID);
               sMsg = "Permanently Ban this Account?\n\n--->" + GetPCPlayerName(oPC);
               break;
            case OPTION_BAN_PERM_CAID:
               SetLocalInt(oPC, BAN_TYPE, SDB_BANTYPE_CAID);
               ...
               break;
            case OPTION_BAN_PERM_PLID:
               SetLocalInt(oPC, BAN_TYPE, SDB_BANTYPE_PLID);
...
void main () {
   ...
   switch(iEvent) {
      ...
      case DLG_PAGE_INIT:
         ...
         SetDlgResponseList(SDB_BAN_LIST, oPC);
         break;
        case DLG_SELECTION:
         HandleSelection();
         break;

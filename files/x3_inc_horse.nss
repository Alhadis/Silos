...
/*
    ...
    the sake of synchronizing animation and the process running in the b,kground,
...
{ // PURPOSE: Delay Command transfer to handle moving inventory
    ...
    { // item found
        ...
        { // to object
            if (GetBaseItemType(oItem)!=BASE_ITEM_CREATUREITEM)
            { // no pchide transfers
                oCopy=SafeCopyItem(oItem,oTo,TRUE);
            } // no pchide transfers
        } // to object
        else
        { // to location
            if (GetBaseItemType(oItem)!=BASE_ITEM_CREATUREITEM)
            { // no pchide transfers
                oCopy=SafeCopyObject(oItem,lLoc);
...
{ // PURPOSE: Set oHorse to be owned by oOwner
    if(HorseGetOwner(oHorse) != oOwner && GetIsObjectValid(HorseGetOwner(oHorse)))
    {
        FloatingTextStringOnCreature("* Dieses Pferd gehört jemand anderem! *", oOwner, FALSE);
        return;
    }

...
//********************************
  ...
  wire		we_MS;
  wire	[25:0]	addr_MS;
  wire	[25:0]	addressBusSlave;
  wire	[31:0]	dataBus_mem_rdata;
  wire	[31:0]	dataBusSlave;
  wire	[7:0]	dataBus_ssp_rdata;
  wire		mem_R;
  wire		mem_W;
...
//anisa  always
//anisa	begin
//anisa		#`NONOVERLAP phi1 = 1;
//anisa		#`PHI_HIGH phi1 = 0;
//anisa		#`NONOVERLAP phi2 = 1;
//anisa		#`PHI_HIGH phi2 = 0;
//anisa	end
...
//anisa	end
 ...
 CMU CMU_inst(	.clk_i		(p_clk),
  ...
  WISHB_MASTER WISHB_MASTER_inst(
		.clk_i		(clk_o),
		.rst_i		(clear_o),	
		.mem_req	(1'b0),
		.mwr_arm	(1'b0),
 ...
 WISHB_SLAVE  WISHB_SLAVE_inst(	
		...
		.ack_o		(ack_SM),
		.cyc_i		(cyc_MS),
		.mem_adr_o	(addressBusSlave),
		.dataBus	(dataBusSlave),
		.mem_r_o	(mem_R),
		.mem_w_o	(mem_W),
		.ssp_sel_o	(ssp_psel),
		.ssp_w_o	(ssp_write));
...
begin
	...
	begin
		ssp_psel_reg3<=ssp_psel_reg2;
	end
		
end

 assign ssp_psel_elongated=ssp_psel|ssp_psel_reg1|ssp_psel_reg2|ssp_psel_reg3;

 assign	dataBusSlave=(ssp_psel_elongated & !ssp_write)?{24'b0,dataBus_ssp_rdata}:32'hZZZZZZZZ;
 assign	dataBusSlave=(!ssp_psel & mem_R)?dataBus_mem_rdata:32'hZZZZZZZZ;
 ...
 SSP	SSP_inst(	
		...
		.PSEL		(ssp_psel),
		.PWRITE		(ssp_write),
		.PWDATA		(dataBusSlave[7:0]),
		.PRDATA		(dataBus_ssp_rdata),
		.SSPOE_B	(SSPOE_B),
		.SSPTXD		(SSP_ser_out),
		.SSPFSSOUT	(SSP_FSS_out),
		.SSPCLKOUT	(SSP_clk_out),
		.SSPFSSIN	(SSP_FSS_out),
		.SSPCLKIN	(SSP_clk_out),
		.SSPRXD		(SSP_ser_out),
		.SSPTXINTR	(SSPTXINTR),
		.SSPRXINTR	(SSPRXINTR));

  memory MEM( 	.dataBus(dataBus_mem_rdata), 
		.addressBus(addressBusSlave), 
		.r(mem_R), 
		.w(mem_W), 
		.phi1(phi1), 
		.phi2(phi2));

# 1 "../../drivers/net/bsd_comp.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../drivers/net/bsd_comp.c"
# 64 "../../drivers/net/bsd_comp.c"
...
# 71 "../../drivers/net/bsd_comp.c" 2



# 1 "../../include/linux/ppp-comp.h" 1
# 43 "../../include/linux/ppp-comp.h"
struct module;
# 63 "../../include/linux/ppp-comp.h"
struct compressor {
 int compress_proto;
...
# 75 "../../drivers/net/bsd_comp.c" 2
# 115 "../../drivers/net/bsd_comp.c"
struct bsd_dict {
    union {
 unsigned long fcode;
 struct {
...
static int bsd_check (struct bsd_db *db)
 ...
 if (db->in_count >= (0x7fffffff>>8) || db->bytes_out >= (0x7fffffff>>8))
   {
...
static void *bsd_alloc (unsigned char *options, int opt_len, int decomp)
  {
    ...
    if (opt_len != 3 || options[0] != 21 || options[1] != 3
 || ((options[2]) >> 5) != 1)
      {
 return ((void *)0);
      }
...
static int bsd_compress (void *state, unsigned char *rptr, unsigned char *obuf,
    int isize, int osize)
  {
    ...
    unsigned char *wptr;
    int olen;
# 615 "../../drivers/net/bsd_comp.c"
    ent = ((((__u8 *)(rptr))[2] << 8) + ((__u8 *)(rptr))[3]);
 ...
 *wptr++ = db->seqno;
 ...
 hval = ((((unsigned long)(c))<<(hshift)) ^ (unsigned long)(ent));
 dictp = &(db->dict[hval]);
...
nomatch:
 ...
 ++db->bytes_out;
    ...
    if (bsd_check(db))
      {
 { bitno -= n_bits; accm |= ((256) << bitno); do { { ++olen; if (wptr) { *wptr++ = (unsigned char) (accm >> 24); if (olen >= osize) { wptr = ((void *)0); } } }; accm <<= 8; bitno += 8; } while (bitno <= 24); };
      }
 ...
 while (finchar > 255)
   {
     ...
     dictp = &(db->dict[dictp2->cptr]);
# 1025 "../../drivers/net/bsd_comp.c"
     *--p = dictp->f.hs.suffix;
     finchar = dictp->f.hs.prefix;
   }
 *--p = finchar;
# 1038 "../../drivers/net/bsd_comp.c"
 if (extra)
   {
     *wptr++ = finchar;
   }
# 1051 "../../drivers/net/bsd_comp.c"
 if (oldcode != 256 && max_ent < db->maxmaxcode)
   {
     ...
     hval = ((((unsigned long)(finchar))<<(db->hshift)) ^ (unsigned long)(oldcode));
     dictp = &(db->dict[hval]);
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) bsdcomp_init(void)
{
...
static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) bsdcomp_cleanup(void)
{
 ppp_unregister_compressor(&ppp_bsd_compress);
}

static initcall_t __initcall_bsdcomp_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = bsdcomp_init;;
static exitcall_t __exitcall_bsdcomp_cleanup __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = bsdcomp_cleanup;;
;
;

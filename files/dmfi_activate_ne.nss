...
void main()
{
    ...
    if (GetStringLeft(sItemTag, 5) == "dmfi_")
    {
        if (!GetIsDM(oUser) &&
            ...
            !GetIsDM(GetMaster(oUser)) && !GetLocalInt(GetModule(), "dmfi_PCUse"))
        {
        ...
        if (sItemTag == "dmfi_rotate")
        {
            object oRotate = oOther;
            if (!GetIsObjectValid(oOther) ||
                GetObjectType(oRotate) != OBJECT_TYPE_PLACEABLE)
                oRotate = GetNearestObjectToLocation(OBJECT_TYPE_PLACEABLE, lLocation);
            if (GetIsObjectValid(oRotate) &&
                GetDistanceBetweenLocations(lLocation, GetLocation(oRotate)) < 0.5)
                AssignCommand(oRotate, SetFacing(GetFacing(oUser)));
            return;
        }

...
void main()
{
    ...
    int nSkill = GetSkillRank(SKILL_CRAFT_MECHANICAL, oPC) - 4*(!GetIsObjectValid(GetItemPossessedBy(oPC, "_mdrn_ot_mechkit")));
    int nRoll = d20();
    ...
    if (GetIsObjectValid(oTarg)==FALSE)
    {
        ...
        object oTrap = CreateTrapAtLocation(TRAP_BASE_TYPE_AVERAGE_TANGLE, lTarg, 2.0, "", STANDARD_FACTION_DEFENDER, "_mdrn_exe_exdis1");
    ...
    else if (GetObjectType(oTarg)==OBJECT_TYPE_DOOR || GetObjectType(oTarg)==OBJECT_TYPE_PLACEABLE)
    {
          if (GetIsTrapped(oTarg))
        {
            ...
            CreateTrapOnObject(TRAP_BASE_TYPE_AVERAGE_TANGLE, oTarg, STANDARD_FACTION_DEFENDER, "_mdrn_exe_exdis1");

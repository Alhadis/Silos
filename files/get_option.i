...
case 'r':
	if (!is_new_arg) break;
	if (strncmp(argv[*argi],"-run-build=",11) == 0) {
		end_option(argv, argi, 11);
		optargs = get_optarg(argv, argi);
		get_next_arg(argi);
		return add_string_option(O_run_buildQ,optargs);
		/* NOTREACHED */
	}
	if (!is_new_arg) break;
...
case 'i':
	...
	if (strncmp(argv[*argi],"-iwithprefixbefore",18) == 0) {
		end_option(argv, argi, 18);
	if (!is_directory(next_string(argv,argi)) && fullwarn)
		warning("%s is not a directory", next_string(argv,argi));
	if (strcmp(next_string(argv,argi),"-default_options")) {
		optargs = get_optarg(argv, argi);
		get_next_arg(argi);
...
case 'f':
	...
	if (!is_new_arg) break;
	if (strncmp(argv[*argi],"-fp-accuracy=",13) == 0) {
		end_option(argv, argi, 13);
		optargs = get_optarg(argv, argi);
		get_next_arg(argi);
		return add_string_option(O_fp_accuracyQ,optargs);
		/* NOTREACHED */
	}
	...
	if (!is_new_arg) break;
	if (strncmp(argv[*argi],"-fb_selective",13) == 0) {
		end_option(argv, argi, 13);
		optargs = get_optarg(argv, argi);
		get_next_arg(argi);
		return add_string_option(O_fb_selective,optargs);
		/* NOTREACHED */
	}
	if (!is_new_arg) break;
...
case 'a':
	...
	if (strcmp(argv[*argi],"-autoscope") == 0) {
		get_next_arg(argi);
		return O_autoscope;
		/* NOTREACHED */
	}
...
case 'W':
	...
	if (strcmp(argv[*argi],"-Wno-extra") == 0) {
		get_next_arg(argi);
		return O_Wno_extra;
		/* NOTREACHED */
	}
	...
	if (strcmp(argv[*argi],"-Wmissing-include-dirs") == 0) {
		get_next_arg(argi);
		return O_Wmissing_include_dirs;
		/* NOTREACHED */
	}
...
case 'D':
	if (!is_new_arg) break;
	if (strncmp(argv[*argi],"-DRIVER:",8) == 0) {
		end_option(argv, argi, 8);
		optargs = get_optarg(argv, argi);
		get_next_arg(argi);
		return add_string_option(O_DRIVER_,optargs);
		/* NOTREACHED */
	}
	if (!is_new_arg) break;
